...
| 左右倾斜设备 | 转向 |
| 向前/向后倾斜 | 加速/减速 |
| M 键 | 开始、暂停、继续；结束后再来一局 |
| L 键 | 音量降低 10% |
| R 键 | 音量提高 10% |
| Q 键 | 标题页切换转向轴/方向；比赛中重新开始 |

开机后的约 0.5 秒会自动标定加速度计中位。此时请按正常游玩姿势平稳握住
设备。BOX2 默认使用屏幕横向对应的 `X+` 控制转向，标题页显示
`STEERING X+ READY` 后即可按 M 开始。如果个别硬件批次的轴向或正反方向不同，
在标题页按 Q 依次切换 `X+ / X- / Y+ / Y- / Z+ / Z-`，选择左右倾斜响应正确的
模式后按 M。HUD 顶部显示车速、分数、音量、生命值和实时倾斜指示。
