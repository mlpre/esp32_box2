...
设备。标题画面显示 `TILT LEFT TO SET` 后，向左明显倾斜一次；游戏会自动识别
传感器在 PCB 上对应的 X/Y/Z 轴和方向。显示 `STEERING X/Y/Z READY` 后即可按 M
开始。这个引导校准避免不同硬件批次或安装方向造成体感无响应、方向相反。
HUD 顶部显示车速、分数、音量、生命值和实时倾斜指示。
